 ...
 */
async function exitUnavailable(subcommand: string): Promise<void> {
  const {
    exitWithError
  } = await import('../utils/process.js');
  exitWithError(`Error: ${subcommand} is unavailable in this checkout.`);
}
...
async function main(): Promise<void> {
  ...
  if (feature('DAEMON') && args[0] === '--daemon-worker') {
    await exitUnavailable('daemon worker');
    return;
  }
  ...
  if (feature('BG_SESSIONS') && (args[0] === 'ps' || args[0] === 'logs' || args[0] === 'attach' || args[0] === 'kill' || args.includes('--bg') || args.includes('--background'))) {
    ...
    enableConfigs();
    await exitUnavailable('background session management');
    return;
  }
  ...
  if (feature('TEMPLATES') && (args[0] === 'new' || args[0] === 'list' || args[0] === 'reply')) {
    ...
    await exitUnavailable('template job commands');
  ...
  if (feature('BYOC_ENVIRONMENT_RUNNER') && args[0] === 'environment-runner') {
    ...
    await exitUnavailable('environment-runner');
    return;
  }
  ...
  if (feature('SELF_HOSTED_RUNNER') && args[0] === 'self-hosted-runner') {
    ...
    await exitUnavailable('self-hosted-runner');
    return;
  }
