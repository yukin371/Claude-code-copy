...
function parseArgs(argv: string[]): SmokeOptions {
  ...
  return { keepTemp, skipStagePublication }
}
...
async function main(): Promise<void> {
  ...
  if (!options.skipStagePublication) {
    ...
    const stagePublication = await runCommand(
      ['bun', 'run', 'scripts/stage-release-publication.ts', '--skip-stage-candidate'],
      repoRoot,
      process.env,
    )
    if (stagePublication.exitCode !== 0) {
      throw new Error(
        `stage-release-publication failed: ${normalize(stagePublication.stderr || stagePublication.stdout)}`,
      )
    }
  }
  ...
  const publicationRoot = join(repoRoot, 'dist', 'release-publication', packageJson.version)
  const metadata = JSON.parse(
    await readFile(join(publicationRoot, 'release-publication.json'), 'utf8'),
  ) as PublicationMetadata
  const publicationServeRoot = resolve(publicationRoot)
  ...
  const server = Bun.serve({
    ...
    fetch(request) {
      ...
      const filePath = resolve(publicationServeRoot, relativePath)

      if (!filePath.startsWith(publicationServeRoot)) {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), 'neko-native-installer-release-publication-'))
  ...
  const installedBinary = join(binDir, metadata.publishedBinary.split('/').pop() ?? 'neko.exe')
  ...
  const doctorHelpResult = await runCommand([installedBinary, 'doctor', '--help'], tempRoot, childEnv)
  const installHelpResult = await runCommand([installedBinary, 'install', '--help'], tempRoot, childEnv)
  ...
  console.log('[PASS] native-installer-release-publication-smoke')
