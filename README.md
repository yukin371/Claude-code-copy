...
- 简化 `providers/models/defaults` 配置已接入 schema、registry、route/source 解析与 transport wiring
- 设置面板与 `ConfigTool` 已开始优先暴露 `defaults.*`
  - `main` 默认模型与 route transport override 已拆开
  - `subagent/frontend/review/explore/plan/guide/statusline` 默认模型已可直接写入 `defaults.*`
- 主查询路径、helper 路径、route helper 已接入 route-aware transport
- `status` / `doctor` 已可显示 route 的 `resolved source`、模型来源（`defaults.*` vs `taskRoutes.*`）和具体配置来源标签
...
- 可读的 route config source 输出，可直接看到 `defaults.main`、`taskRoutes.review.provider`、对应 env 名等来源标签
